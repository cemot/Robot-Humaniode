...
##	root.title('Servos')
	...
	nombre_str2.set("home");
	nombre_entry2.grid(row=2,column=8)

	# CAMPO 2 : boton ejecutar
##	ejecutar = Button(master,text="Ejecutar movimiento de servos",command=valores_servos,relief=FLAT)
##	ejecutar.grid(row=3,column=7)
	# CAMPO 3 : 
	real = Button(master,text="Cambiar valores GUI y ejecutar",command=cambiarValoresYEjecutar,relief=FLAT,state=ACTIVE, repeatdelay=1, repeatinterval=1)
	real.grid(row=3,column=7)
	# CAMPO 4 : 
	crear = Button(master,text="Crear un nuevo archivo",command=crear_archivo,relief=FLAT)
	crear.grid(row=4,column=7)
	#CAMPO 5: 
	real = Button(master,text="Ejecutar getup",command=modo_real,relief=FLAT,state=ACTIVE, repeatdelay=1, repeatinterval=1)
	real.grid(row=5,column=7)
        #CAMPO 6> 	
	agregar = Button(master,text="Agregar rutina de valores al archivo",command=agregar_datos,relief=FLAT)
	agregar.grid(row=6,column=7)
	home()
	mainloop()
