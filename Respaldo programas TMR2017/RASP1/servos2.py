...
def modo_real():
	...
	print "Archivo: "+arch
        ...
        for x in range(1,50):
          posicion = getPositions("chiripazo")
          setPositionRobot2(posicion)
	

          posicion = getPositions("homechi")
          setPositionRobot2(posicion)
          time.sleep(.5)

       

        """posicion = getPositions("getup0")
	setPositionRobot2(posicion)
	time.sleep(3)
...
while True:
	...
	#CAMPO 5: boton getup
	real = Button(root,text="Ejecutar getup",command=modo_real,relief=FLAT,state=ACTIVE, repeatdelay=1, repeatinterval=1)
	real.grid(row=6,column=1)
        #CAMPO 6> BOTON CAMBIAR VALORES	
